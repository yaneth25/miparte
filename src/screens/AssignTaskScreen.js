...
import { COLORS, DESCRIPTION_MAX_LENGTH, RADIUS, SPACING } from '../constants/theme';
import { useTasks } from '../context/TasksContext';

const SHORT_FIELD_MAX_LENGTH = 50;

export function AssignTaskScreen({ navigation, route }) {
  const { addTask, updateTask } = useTasks();
  
  // Detectamos si recibimos una tarea para editar
      ...
      <Modal visible={isMenuOpen} transparent animationType="fade">
        <Pressable style={styles.menuOverlay} onPress={() => setIsMenuOpen(false)}>
          <View style={styles.sideMenu}>
            <View style={styles.menuHeader}><Text style={styles.menuTitle}>☰ MENÚ</Text></View>
            <View style={styles.menuOptions}>
               {['CUENTA', 'TAREAS', 'HORARIO', 'NOTIFICACIONES', 'CONFIGURACIÓN'].map(item => (
                 <Pressable key={item} style={styles.menuItem} onPress={() => setIsMenuOpen(false)}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.inner}>
          ...
          <View style={styles.actionsRow}>
            <Pressable onPress={() => navigation.goBack()} style={styles.actionPill}><Text style={styles.actionText}>CANCELAR</Text></Pressable>
